...
pair<vector<Point>, int> dfs(const Point &start, const Point &target) {
    ...
    while (!s.empty()) {
        ...
        // Обрабатываем соседей (в обратном порядке для более естественного DFS)
        const vector<Point> &neighbors = graph[current];
        for (auto it = neighbors.rbegin(); it != neighbors.rend(); ++it) {
            const Point &neighbor = *it;
            if (!visited[neighbor]) {
                s.push(neighbor);
    ...
    cout << "Посещено вершин (DFS): " << visited_count << endl;
